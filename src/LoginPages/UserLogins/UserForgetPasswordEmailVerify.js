...
import { resetverifyemailerrorMessage, userEmailVerfication, userForgetPasswordEmailVerfication, userResendVerifyCode } from "../../Redux/slices/AuthSlice";
...
const UserForgetPasswordVerification = () => {
...
const [dataForm, setDataForm] = useState({
  ...
  const { loading, error, newVeryCode, verifyemailerrorMessage, newVerifyErrorMessage, successVerifyMessage, forgetverify } = useSelector((state) => state.auth);
  console.log(successVerifyMessage, forgetverify);


console.log(forgetverify, newVerifyErrorMessage);

  const [showPassword, setShowPassword] = useState(false);
 useEffect(() => {
    if (forgetverify) {
              localStorage.setItem("ruauserregisteredEmail", forgetverify.data.email);
      localStorage.setItem("ruaUserToken", forgetverify.data.api_token);
      localStorage.setItem('ruaUserDetails', JSON.stringify(forgetverify?.data)); 
      navigate("/usersPasswordCreation");
    }
  }, [forgetverify, navigate]);
...
const handleSubmit = (e) => {
  ...
  if (isValid && email && formData.code) {
    ...
    dispatch(userForgetPasswordEmailVerfication(payload)); // Replace `verifyUser` with your actual verify action
  }
};
...
export default UserForgetPasswordVerification;
