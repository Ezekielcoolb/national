...
import { fetchFlagedItemsByUser, fetchLodgedItemUser, fetchMyComplains } from "../Redux/slices/secondUserSlice";
...
const UserFlagedItem = () => {
     const dispatch = useDispatch()
        const { listOfUserFlagedItems, loading, success, error } = useSelector((state) => state.otherUser);
      console.log(listOfUserFlagedItems);
      
  const navigate = useNavigate();
  const complains = listOfUserFlagedItems?.data?.data
  ...
  useEffect(() => {
      dispatch(fetchFlagedItemsByUser());
    }, [dispatch]);
                  ...
                  {CurrentComplains ? (
                    CurrentComplains.map((caseItem, index) => {
                      return (
                        ...
                        >
                          <td>{index + 1}</td>
                           <td>{caseItem.lodge_item.item_ref}</td>
        <td>{caseItem.lodge_item.item_name}</td>
                          <td>{caseItem.lodge_item.vehicle_bike_number}</td>
                           <td>{caseItem.lodge_item.rider_phone_number}</td>
                            <td>{caseItem.lodge_item.rider_company}</td>
                          
                          <td>{caseItem?.lodge_item.destination_address?.split(' ').slice(0, 5).join(' ')}{caseItem?.description?.split(' ')?.length > 5 ? '...' : ''}</td>
